...
To build an RPM, run
    ...
    rpmbuild -ba rpm/chronos.spec --define "_topdir `pwd`" --define "_version `cat version`" --define "_git_hash `git rev-parse --short HEAD`" --define "_datestamp `date +%y%m%d.%H%M%S`"
